                        ...
                        {memberships.map((m) => (
                          ...
                          >
                            ...
                            <td className="py-2.5 px-2 text-slate-600 text-xs">
                              {MEMBERSHIP_TYPE_KEY[m.type] || m.type}
                            </td>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* 名称 */}
        <Field label={'卡名'} required error={errors.name}>
          <input
            type="text"
          ...
          >
            ...
            {memberships.map((m) => (
              <option key={m.id} value={m.id}>
                {m.name}（{MEMBERSHIP_TYPE_KEY[m.type]} / ¥{m.price} / {m.durationDays}天）
              </option>
            ))}
          </select>
        </Field>
